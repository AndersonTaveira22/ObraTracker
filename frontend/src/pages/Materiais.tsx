...
export default function Materiais() {
  ...
  const [formData, setFormData] = useState({ nome: '', quantidade: 1, valor_unitario: 0 });
  const [saving, setSaving] = useState(false);

  const userId = 'user-1';

  const loadData = () => {
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3333/api'}/materiais/${userId}`)
      .then(res => res.json())
      .then(data => {
        setMateriais(data || []);
        setLoading(false);
  ...
  const handleAdd = async () => {
    if (!formData.nome || !formData.quantidade || !formData.valor_unitario) return alert('Preencha os campos obrigatórios');
    setSaving(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3333/api'}/materiais`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...formData, quantidade: Number(formData.quantidade), valor_unitario: Number(formData.valor_unitario), userId })
      });
      if (res.ok) {
        setShowAdd(false);
        setFormData({ nome: '', quantidade: 1, valor_unitario: 0 });
        loadData();
      }
    } catch(e) {
      console.error(e);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Estoque e custos de materiais da obra.</p>
        </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl border border-gray-100">
            <h3 className="text-xl font-bold mb-4 text-gray-800">Adicionar Material</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nome do Material</label>
                <input type="text" value={formData.nome} onChange={e => setFormData({...formData, nome: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 outline-none focus:border-blue-500 transition" placeholder="Cimento CP II 50kg" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" step="0.01" value={formData.valor_unitario || ''} onChange={e => setFormData({...formData, valor_unitario: parseFloat(e.target.value)})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 outline-none focus:border-blue-500 transition" placeholder="32.50" />
                </div>
              </div>
            </div>
            <div className="flex justify-end mt-6 gap-3">
              ...
              <button disabled={saving} onClick={handleAdd} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl transition shadow-sm font-medium">
                {saving ? 'Registrando...' : 'Salvar Material'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-x-auto min-w-full">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-400">
            <p className="mb-4 text-lg">Nenhum material registrado ainda.</p>
            <button onClick={() => setShowAdd(true)} className="text-blue-600 font-medium hover:underline">Registrar material</button>
          </div>
        ) : (
            ...
            <tbody>
              {materiais.map(item => (
                <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                  ...
                  <td className="p-4 text-gray-500">R$ {item.valor_unitario.toFixed(2)}</td>
                  <td className="p-4 font-semibold text-gray-800">R$ {(item.quantidade * item.valor_unitario).toFixed(2)}</td>
